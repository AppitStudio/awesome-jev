# Add a reviewable Jev decision gate to a LangChain agent
...
3. Configure `AutoModeMiddleware(tools=[...])` for the specific tool objects. Its risk classification can return an error `ToolMessage` instead of calling a tool. Add an independent approval checkpoint for any action that requires consent; the middleware does **not** ask for approval itself.
...
On 25 September 2026 we read the full article and its visible examples, the current [LangChain integration documentation](https://docs.langchain.com/oss/python/integrations/providers/typesafe) and [TypeSafe quick start](https://docs.typesafe.ai/introduction/quickstart), plus the two pinned catalog project pages. We ran the repository's Support Router with six **authored synthetic tickets**: three automatic suggestions, three human-review records and no errors. That exercise validates the example's deterministic review flow only. We did **not** execute this LangChain harness, call TypeSafe, measure latency or cost, or evaluate safety on a real trace dataset. The article showed an edited indicator; we could not verify its edit history. Recheck experimental middleware APIs before implementation.

## Adoption questions

### Does Auto Mode ask a person to approve a risky call?
