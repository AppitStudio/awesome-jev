...
- [Jev Belay](https://github.com/valentynkit/jev-belay) - Claude Code Stop hook that reads the transcript for evidence and spends one Jev call only when files changed with no passing check since, before letting an unverified "done" stand. [Project guide](community/projects/jev-belay.md).
- [Jev Commit](https://github.com/valentynkit/jev-commit) - Pre-commit hook where one Jev call checks whether the commit message matches the staged diff, plus debug leftovers and unmentioned work; blocks only on a leaked credential. [Project guide](community/projects/jev-commit.md).
...
- [jev.nvim](https://github.com/valentynkit/jev.nvim) - Asks a Neovim buffer a plain-language question; Treesitter splits it into functions, Jev scores each one, and answers land in quickfix ranked by probability. [Project guide](community/projects/jev-nvim.md).
